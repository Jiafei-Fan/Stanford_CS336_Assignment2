...
run_config() {
  ...
  echo "[run.sh] Start ${model_size}: d_model=${d_model}, d_ff=${d_ff}, num_layers=${num_layers}, num_heads=${num_heads}, context_length=${context_length}, mixed_precision=${mixed_precision}, write_benchmark_csv=${write_benchmark_csv}, dump_memory_snapshot=${dump_memory_snapshot}"
...
if [[ "${run_feedfoward}" -eq 1 && "${run_both_feedfoward_backward}" -eq 1 ]]; then
  echo "[run.sh] Choose only one: run_feedfoward=1 or run_both_feedfoward_backward=1."
  exit 1
fi
...
if [[ "${any_run}" -eq 0 ]]; then
  echo "[run.sh] No model is selected. Set one or more run_* flags to 1 and rerun."
fi
