...
@torch.compile
def compiled_transformer_forward(
...
BENCHMARK_HISTORY_CSV = RESULTS_DIR / "benchmark_history_compiled.csv"
        ...
        if timing_pass == "forward":
            with torch.no_grad():
                with bf16_autocast_context(device, mixed_precision_enabled):
                    _ = compiled_transformer_forward(model, inputs)
            return
        # For "forward_backward", include backward but not the optimizer update.
        model.zero_grad(set_to_none=True)
        with bf16_autocast_context(device, mixed_precision_enabled):
            logits = compiled_transformer_forward(model, inputs)
...
def train(config: TrainConfig) -> None:
    ...
    try:
        ...
        for step in range(config.total_steps):
            ...
            with bf16_autocast_context(device, mixed_precision_enabled):
                logits = compiled_transformer_forward(transformer_lm, inputs)
            loss = cross_entropy(logits.float(), targets) if mixed_precision_enabled else cross_entropy(logits, targets)
